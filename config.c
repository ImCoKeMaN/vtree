...
void load_config() {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        // [Paths]
        else if (strcmp(k, "StartDirectoryLeft") == 0 || strcmp(k, "StartDirectoryRight") == 0) {
            char *target = (strcmp(k, "StartDirectoryLeft") == 0) ? cfg.start_left : cfg.start_right;
            char *resolved = realpath(v, NULL);
            if (resolved) {
                strncpy(target, resolved, MAX_PATH - 1);
                target[MAX_PATH - 1] = '\0';
                free(resolved); 
            } else {
                strncpy(target, v, MAX_PATH - 1);
                target[MAX_PATH - 1] = '\0';
            }
        }
